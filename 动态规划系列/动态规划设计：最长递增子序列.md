# 动态规划设计：最长递增子序列
[leetcode 300: Longest Increasing Subsequence](https://leetcode-cn.com/problems/longest-increasing-subsequence/submissions/)
...
class Solution:
    def lengthOfLIS(self, nums: List[int]) -> int:
        ...
        for i, n in enumerate(nums):
            top_cards = [p[-1] for p in piles]
            ins_pos = bisect(top_cards, n)
            if len(piles) == ins_pos:
                # add new pile
                piles.append([n])
            else:
                piles[ins_pos].append(n)
        return len(piles)
```
