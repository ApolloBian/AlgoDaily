...
class Solution:
    ...
    def minDistance(self, word1: str, word2: str) -> int:
        ...
        if key not in self.dp:
            if 0 in key: result = sum(key) # trivial case
            elif word1[-1] == word2[-1]: result = self.minDistance(word1[:-1], word2[:-1])
            else:
                result = 1 + min(
                    self.minDistance(word1[:-1], word2[:-1]), # replace
                    self.minDistance(word1[:-1], word2), # add
                    self.minDistance(word1, word2[:-1]), # delete
                )
            self.dp[key] = result
        return self.dp[key]
```
...
class Solution:
    def minDistance(self, word1: str, word2: str) -> int:
        row1 = [i for i in range(len(word2)+1)]
        row2 = [1] * len(row1)
        for i, w1 in enumerate(word1, start=2):
            for j, w2 in enumerate(word2, start=1): # fill row2
                if w2 == w1: row2[j] = row1[j-1]
                else: row2[j] = 1 + min(row2[j-1], row1[j], row1[j-1])
            row1 = row2 # move down
            row2 = [i] * len(row1)
        return row1[-1]
```